...
const UpdateSaleDetail = ({id, newSaleDetailAmount, newSaleDetailUnitPrice, newSaleDetailSaleId, newSaleDetailProductId}: {id: number, newSaleDetailAmount: number, newSaleDetailUnitPrice:number, newSaleDetailSaleId:number, newSaleDetailProductId:number}) => {
  ...
  const handleInputChangeUnitPrince = (e: React.ChangeEvent<HTMLInputElement>) => {
          ...
          {showModal &&(
            <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
              <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                <h3 className="text-lg font-medium mb-4 text-center">Editar detalle de Venta</h3>
                <form >
                  <div className="mb-4">
                    <label htmlFor="total" className="block text-gray-700 text-sm font-medium">Cantidad:</label>
                    
                    <input type="number" value={newAmount} onChange={handleInputChangeAmount}  className="w-full h-10 p-4 border rounded-xl" placeholder="Ingrese la cantidad"/> 
                    <label htmlFor="total" className="block text-gray-700 text-sm font-medium">Precio Unitario:</label>

                    <input type="number" value={newUnitPrice} onChange={handleInputChangeUnitPrince}  className="w-full h-10 p-4 border rounded-xl" placeholder="Ingrese la cantidad"/> 

                  </div>
                  <div className="mb-4">
                    <label htmlFor="saleId" className="block text-gray-700 text-sm font-medium">Venta:</label>
                    <select name="saleId" onChange={(e) => handleSelectChangeSale(e)} value={newSaleId} className="w-full border-gray-300 rounded-lg px-3 py-2 mb-4" >
                  ...
                  <div>
                     <label htmlFor="productId" className="block text-gray-700 text-sm font-medium">Seleccion un iD:</label>

              <select id="productId" name="productId" onChange={(e) => handleSelectChangePId(e)} value={newProductId} className="w-full border-gray-300 rounded-lg px-3 py-2 mb-4" >
                  <option value="" disabled>Selecciona un Id </option>
