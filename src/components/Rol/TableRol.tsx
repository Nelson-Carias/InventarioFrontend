...
export default function TableRol() {
  const { roles, OnGetRoles, OnDeleteRol, pagination_roles } = useRoleStore();
  ...
  const handleNext = () => {
    console.log("Current Page:", pagination_roles.currentPag);
    console.log("Total Pages:", pagination_roles.totalPag);

    if (pagination_roles.currentPag < pagination_roles.totalPag) {
      OnGetRoles(pagination_roles.currentPag + 1, displayCount, "");
    }
  };
  ...
  const handlePrev = () => {
    ...
    if (pagination_roles.currentPag > 1) {
      OnGetRoles(pagination_roles.currentPag - 1, displayCount, "");
    }
  };

  console.log("Is Prev Button Disabled:", pagination_roles.currentPag === 1);
  console.log(
    "Is Next Button Disabled:",
    pagination_roles.currentPag === pagination_roles.totalPag
  );
  ...
  return (
    <>
      ...
      <div className="flex items-end justify-end">
        <CreateRol
          onRolCreated={() => OnGetRoles(1, displayCount, "")}
        ></CreateRol>
      </div>
      ...
      {roleToDelete && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className=" bg-white p-4 rounded-lg shadow-lg">
            <p>
              ¿Estas seguro que quieres eliminar el rol "{roleToDelete.roleName}
              "?
            </p>
      ...
      <div className="pagination-controls flex items-center justify-center space-x-4">
        <button
          className={`px-4 py-2 border rounded-full shadow-md transition-transform transform hover:scale-105 ${
            pagination_roles.currentPag === 1
              ? "bg-gray-300 text-gray-500 cursor-not-allowed"
              : "bg-blue-500 text-white hover:bg-blue-700"
          }`}
          onClick={handlePrev}
          disabled={pagination_roles.currentPag === 1}
        >
          Atras
        </button>
        <span className="text-xl font-semibold text-gray-700">
          Pagina {pagination_roles.currentPag} de {pagination_roles.totalPag}
        </span>
        <button
          className={`px-4 py-2 border rounded-full shadow-md transition-transform transform hover:scale-105 ${
            pagination_roles.currentPag === pagination_roles.totalPag
          ...
          onClick={handleNext}
          disabled={pagination_roles.currentPag === pagination_roles.totalPag}
        >
          Siguiente
        </button>
      </div>
