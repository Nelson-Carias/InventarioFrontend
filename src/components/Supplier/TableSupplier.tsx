...
export default function TableRol() {
  ...
  const [supplierToDelete, setSupplierToDelete] = useState<{id: number; supplierName: string, supplierContact:number, supplierDirection:string} | null>(null)
  
  useEffect(() =>{
    OnGetSupplier();
  }, [])

  const handleDelete = (id: number, supplierName: string, supplierContact:number, supplierDirection:string) => {
