...
import { ICreateSupplier } from '../../types/supplier.types';
// import { useRoleStore } from '../store/rol.store';
import { create_supplier} from '../../service/supplier.service';
...
export default function CreateSupplier(){
 ...
 const {  handleSubmit, register, reset } = useForm<ICreateSupplier>();
 ...
 const onSubmit = (data: ICreateSupplier) => {
  console.log(data)
  create_supplier(data)
    .then((data) => {
      console.log('Proveedor agregado con éxito:', data);
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="mb-4">
                <label htmlFor="suppplier" className="block text-gray-700 text-sm font-medium">Supplier:</label>
                <input  {...register("name")} className="w-full h-10 p-4 border rounded-xl" placeholder="Ingrese el nombre del Proveedor"/> 
               <br/>
               <br/>
                <input  {...register("contact")} className="w-full h-10 p-4 border rounded-xl" placeholder="Ingrese el contacto del Proveedor"/> 
              
                <input  {...register("direction")} className="w-full h-10 p-4 border rounded-xl mt-4" placeholder="Ingrese la direccion del  Proveedor"/> 
